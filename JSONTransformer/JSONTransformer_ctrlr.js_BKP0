...
i2b2.JSONTransformer.qmDropped = function(sdxData) {
	...
	$("JSONTransformer-QMDROP").innerHTML = i2b2.h.Escape(sdxData.sdxInfo.sdxDisplayName);
...
i2b2.JSONTransformer.getResults = function() {
	if (i2b2.JSONTransformer.model.dirtyResultsData) {
		...
		var msg_filter = '<input_list>\n' +
			...
			'		<patient_set_coll_id>'+i2b2.JSONTransformer.model.qmRecord.sdxInfo.sdxKeyValue+'</patient_set_coll_id>\n'+
			'	</patient_list>\n'+
			'</input_list>\n'+
			'<filter_list />\n'+
			'<output_option>\n'+
		...
		scopedCallback.callback = function(results) {
			...
			i2b2.JSONTransformer.model.sumCounts = eval("(" + Object.toJSON(hData) +")"); 
			var s = '';
			...
			s += '<div class="resultVal">' + i2b2.JSONTransformer.model.qmRecord.origData.size + '</div>';
			s += '</div>'
			...
			for (var DemCat in DemCats) {
				var DemCatVals = [];
				var maxVal = 0;
				for (var DemCatVal in i2b2.JSONTransformer.model.sumCounts[DemCat]) {
					var tempVal = i2b2.JSONTransformer.model.sumCounts[DemCat][DemCatVal];
			...
			$$("DIV#JSONTransformer-mainDiv DIV#JSONTransformer-TABS DIV.results-finished")[0].innerHTML = s;
		...
		i2b2.CRC.ajax.getPDO_fromInputList("Plugin:JSONTransformer", {PDO_Request: msg_filter}, scopedCallback);
	}
}
